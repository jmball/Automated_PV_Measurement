...
# slashes
folderpath = r'C:/SolarSimData/James B/2016/07-Jul/21-07-2016 Test/'
filepath = r'TEST_LOG.txt'
...
for i in range(len(var_names)):
    if var_names[i] != 'Control':
        ...
        plt.figure(i + 2 * i, figsize=(13, 10), dpi=300)
        ...
        plt.savefig(log_file.replace('.txt', var_names[i] +
                                     '_basic_boxplots.png'))

        plt.figure(i + 1 + 2 * i, figsize=(13, 5), dpi=300)
...
# create main figure
plt.figure(figsize=(14, 10.5), dpi=300)
plt.suptitle('JV scans of every pixel', fontsize=14, fontweight='bold')
i = 1
...
for file in best_pixels['File Path']:
    ...
    JV_light_HL_data = JV_light_HL_data[~np.isnan(JV_light_HL_data).any(
        axis=1)]
    JV_dark_LH_data = JV_dark_LH_data[~np.isnan(JV_dark_LH_data).any(axis=1)]
    JV_dark_HL_data = JV_dark_HL_data[~np.isnan(JV_dark_HL_data).any(axis=1)]
